# Technical Documentation for Independent Assessment
## CloudCraft - Interview Question Bank Platform

**Project Repository:** CloudCraft
**Assessment Date:** February 2026
**Domain:** 
**Architecture:** Serverless Full-Stack Web Application on AWS
...
CloudCraft is a serverless full-stack web application for interview preparation, featuring:

- **Serverless Backend**: Python Lambda functions with DynamoDB
- **AI Integration**: AWS Bedrock (Claude 3.7 Sonnet) for answer evaluation
...
- **Monitoring**: CloudWatch, CloudTrail, and custom dashboards
...
|-----------|-----------|---------|
| **Frontend Hosting** | S3 + CloudFront | Static React SPA with CDN |
| **API Gateway** | AWS API Gateway | RESTful API with Cognito authorization |
| **Compute** | AWS Lambda (Python 3.11) | Serverless backend functions |
| **Database** | DynamoDB | NoSQL question storage |
| **Authentication** | AWS Cognito | User management & JWT tokens |
| **AI Service** | AWS Bedrock (Claude 3.7 Sonnet) | Answer evaluation |
| **DNS** | Route 53 + Supernova | Custom domain management |
| **Certificates** | AWS Certificate Manager | SSL/TLS certificates |
| **Monitoring** | CloudWatch + CloudTrail | Metrics, logs, and audit trails |
...
**Purpose**: Main application interface for question browsing and AI practice
...
The project uses GitHub Actions with OpenID Connect (OIDC) for secure AWS access.
...
- Paths: `infrastructure/**, backend/**, package.json`
...
**Test Files:**
- `backend/tests/test_questions_handler.py`
...
**Decision**: Use AWS Lambda + DynamoDB instead of EC2 + RDS
...
**Decision**: Use AWS CDK (TypeScript) instead of CloudFormation or Terraform

**Rationale:**
- Type safety and IDE autocomplete
- Programming language benefits (loops, conditionals, functions)
...
**Decision**: Separate Alpha and Production accounts
...
- Security isolation
- Integration testing in Alpha before Production

### 4. CI/CD with GitHub Actions

**Decision**: Use GitHub Actions instead of Jenkins or CodePipeline
...
### 6. AWS Bedrock for AI

**Decision**: Use Bedrock instead of OpenAI API
...
- Cost predictability
- Same region as application (latency)

### 7. Cognito for Authentication

**Decision**: Use Cognito instead of Auth0 or custom auth
...
- CloudFront PriceClass 100 (US/Europe)

**Sustainability:**
- Serverless minimizes idle resources
- CloudFront edge caching reduces data transfer
- S3 lifecycle policies (Infrequent Access)
...
1. **CORS Configuration**: Currently allows all origins (`ALL_ORIGINS`)
   - **Mitigation**: Should be locked to specific domains in production

2. **Rate Limiting**: Not explicitly configured at API Gateway level
   - **Mitigation**: Add API Gateway throttling per endpoint

3. **CloudFront Caching**: Not optimized for API responses
   - **Mitigation**: Consider Lambda@Edge for dynamic caching

4. **Database Backups**: Relies on DynamoDB point-in-time recovery
   - **Mitigation**: Add AWS Backup for scheduled snapshots

5. **Monitoring Gaps**: No front-end error tracking
   - **Mitigation**: Integrate Sentry or CloudWatch RUM

### Planned Enhancements

1. **Advanced AI Features:**
   - Multi-turn conversation with Marcus
...
This technical documentation provides comprehensive evidence of all techniques and implementations used in the CloudCraft (Interview Question Bank) project. The application demonstrates:

✅ **Modern Serverless Architecture**: Lambda, DynamoDB, API Gateway, CloudFront
✅ **AI Integration**: AWS Bedrock (Claude 3.7 Sonnet) for answer evaluation
✅ **Infrastructure as Code**: AWS CDK with TypeScript for type-safe IaC
✅ **CI/CD Automation**: GitHub Actions with OIDC, Trivy scanning, and multi-environment deployment
✅ **Security Best Practices**: Cognito authentication, IAM least privilege, encryption, audit logging
✅ **Monitoring & Observability**: CloudWatch alarms, dashboards, structured logging, CloudTrail
✅ **Testing Strategy**: Unit tests, integration tests, infrastructure tests, security scanning
✅ **Multi-Environment**: Separate Alpha and Production accounts with subdomain delegation

**Key Technologies:**
- **Backend**: Python 3.11, AWS Lambda, boto3, DynamoDB, Bedrock
- **Frontend**: React 19, TypeScript, Vite, AWS Amplify
- **Infrastructure**: AWS CDK (TypeScript), CloudFormation
- **CI/CD**: GitHub Actions, Trivy, pytest, Jest
- **Security**: Cognito, IAM, ACM, CloudTrail, encryption
- **Monitoring**: CloudWatch, SNS, structured logging

**Deployment Evidence:**
- **Production**: 
- **Alpha**: 
- **CloudFormation Stacks**: ServiceStack (both environments)
- **CI/CD**: Automated deployments via GitHub Actions
- **Security**: Trivy scanning, OIDC authentication, manual production approval

This documentation, combined with the source code in the repository, provides comprehensive evidence of implementation for independent assessment.
...
**Repository**: github.com/apaps/CloudCraft
